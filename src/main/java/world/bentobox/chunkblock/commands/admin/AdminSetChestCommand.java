...
import world.bentobox.chunkblock.oneblocks.OneBlockObject.Rarity;
...
public class AdminSetChestCommand extends CompositeCommand {
    ...
    @Override
    public void setup() {
        setParametersHelp("chunkblock.commands.admin.setchest.parameters");
        setDescription("chunkblock.commands.admin.setchest.description");
        // Permission
    ...
    public boolean canExecute(User user, String label, List<String> args) {
        ...
        if (opPhase.isEmpty()) {
            user.sendMessage("chunkblock.commands.admin.setchest.unknown-phase");
            return false;
        } else {
            phase = opPhase.get();
        }
        ...
        try {
            ...
            user.sendMessage("chunkblock.commands.admin.setchest.unknown-rarity");
            return false;
        }
        ...
        if (!target.getType().equals(Material.CHEST)) {
            user.sendMessage("chunkblock.commands.admin.setchest.look-at-chest");
            return false;
        }
        chest = (Chest)target.getState();
        if (chest.getInventory().getHolder() instanceof DoubleChest) {
            user.sendMessage("chunkblock.commands.admin.setchest.only-single-chest");
            return false;
        }
        return true;
    }
    ...
    public boolean execute(User user, String label, List<String> args) {
        ...
        if (items.isEmpty()) {
            user.sendMessage("chunkblock.commands.admin.setchest.chest-is-empty");
            return false;
        }
        phase.addChest(items, rarity);
        if (addon.getOneBlockManager().savePhase(phase)) {
            user.sendMessage("chunkblock.commands.admin.setchest.success");
        } else {
            user.sendMessage("chunkblock.commands.admin.setchest.failure");
        }
        return true;
    }
