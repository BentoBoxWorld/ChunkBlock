...
public class JoinLeaveListener implements Listener {
     ...
     */
    public JoinLeaveListener(ChunkBlock addon) {
        this.addon = addon;
    }
        ...
        if (island != null) {
            addon.getBlockListener().saveIsland(island).thenAccept(r -> {
                if (Boolean.FALSE.equals(r)) {
                    addon.logError("Could not save ChunkBlock island at " + Util.xyz(island.getCenter().toVector()) + " to database " + island.getUniqueId());
                }
            });
        }
    }
